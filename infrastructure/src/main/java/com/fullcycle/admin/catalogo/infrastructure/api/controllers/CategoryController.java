...
public class CategoryController implements CategoryAPI {
    ...
    public ResponseEntity<?> createCategory(CreateCategoryRequest input) {
        ...
        final var command = CreateCategoryCommand.with(input.name(),
            input.description(),
            input.active() != null ? input.active() : true);

        final Function<Notification, ResponseEntity<?>> onError = notifications ->
            ResponseEntity.unprocessableEntity().body(notifications);

        final Function<CreateCategoryOutput, ResponseEntity<?>> onSuccess = output -> ResponseEntity.created(URI.create("/categories/" + output.id()))
            .body(output);
    ...
    @Override
    public ResponseEntity<?> updateCategory(final String id,
                                            final UpdateCategoryRequest input) {
        final var command = UpdateCategoryCommand.with(id,
            input.name(),
            input.description(),
            input.active() != null ? input.active() : true);
